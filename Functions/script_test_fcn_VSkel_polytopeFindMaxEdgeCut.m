% script_test_fcn_VSkel_polytopeFindMaxEdgeCut
% Tests function: fcn_VSkel_polytopeFindMaxEdgeCut
...
assert(length(max_edge_cuts(:,1)) == length(vertices(:,1)));
assert(length(max_edge_cuts(1,:)) == 1);

% Check that all values make sense
assert(all(max_edge_cuts>=0));
...
max_edge_cuts = fcn_VSkel_polytopeFindMaxEdgeCut(vertices, unit_normal_vectors, unit_vertex_projection_vectors, ([]));
